...
@MODELS.register_module()
class DefaultKeypointer(nn.Module):
    def __init__(self, backbone=None, criteria=None):
        super().__init__()
        self.backbone = build_model(backbone)
        self.criteria = build_criteria(criteria)
    ...
    def forward(self, input_dict):
        if "condition" in input_dict.keys():
            ...
            # currently, only support one batch one condition
            input_dict["condition"] = input_dict["condition"][0]
        kp_logits = self.backbone(input_dict)

        # train
        if self.training:
            loss = self.criteria((kp_logits, input_dict["offset"]), input_dict["kp_index"])
            return dict(loss=loss)
        # eval
        elif "kp_index" in input_dict.keys():
            loss = self.criteria((kp_logits, input_dict["offset"]), input_dict["kp_index"])
            return dict(loss=loss, kp_logits=kp_logits)
        # test
        else:
            return dict(kp_logits=kp_logits)